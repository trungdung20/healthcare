...
def index(request,extra_context=None, template=None):
	...
	if patient:
		...
		if count > 5:
			...
			doctor_suggestion_list = Doctor.objects.filter(id_in=rand_ids)
...
def profile_patient(request, patient_id): 
	...
	context_dict['topic_follow'] = topics_follow 
	context_dict['patient'] = patient 
...
def patient_notification_request(request): # display friend request from doctor to patient 
	...
	list = []
	for doctor_unrejected_request, doctor_rejected_request, doctor_accepted_request, advisor_accepted,advisor_rejected, answer_alert,patient_answer_follow,patient_doctor_add_topic,patient_doctor_edit_topic,patient_doctor_add_checklist,patient_doctor_edit_checklist,add_related_topic_follow,edit_topic_follow,add_answer_topic_follow,add_question_topic_follow in itertools.izip_longest(doctor_unrejected_request_list,doctor_rejected_request_list,doctor_accepted_request_list,advisor_accepted_list,advisor_rejected_list,answer_alert_list,patient_answer_follow_list, patient_doctor_add_topic_list, patient_doctor_edit_topic_list, patient_doctor_add_checklist_list , patient_doctor_edit_checklist_list,add_related_topic_follow_list,edit_topic_follow_list,add_answer_topic_follow_list,add_question_topic_follow_list):
		dict = {}
...
def main_search(request):
	...
	question_result_list = Question.objects.filter(title__icontains=search_index)
...
def patient_add_condition(request):
	...
	c = RequestContext(request,{'patient_condition_form':patient_condition_form,'patient_id':patient_id})
	html = new_template.render(c)
...
def patient_add_medication(request):
	...
	new_template = loader.get_template('icare/patient_edit/patient_medication_form.html')
	c = RequestContext(request,{'patient_medication_form':patient_medication_form,'patient_id':patient_id})
	html = new_template.render(c)
...
def patient_add_family(request):
	...
	new_template = loader.get_template('icare/patient_edit/patient_family_form.html')
	c = RequestContext(request,{'patient_family_form':patient_family_form,'patient_id':patient_id})
	html = new_template.render(c)
...
def add_question_related(request,question_id):
	...
	if request.method == "POST":
		...
		if question_form.is_valid():
			...
			relation1 = QuestionRelate.objects.get_or_create(question=question,related_question=related_question)
